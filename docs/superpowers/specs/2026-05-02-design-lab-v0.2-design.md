...
│   ├── case-loader.ts             # 大改 → 遷 .ts（recursive scan clients/*/cases/ + clients/*/anti-library/，含 client + sentiment filter）
│   ├── case-writer.ts             # 大改 → 遷 .ts（依 sentiment 分流：positive → clients/<slug>/cases/，negative → clients/<slug>/anti-library/）
...
`_personal/meta.yaml` 是 init 時自動建的預設客戶（v0.1 migration 來的 case 也歸這裡）：
...
**重要：`_personal` 不是「特殊 union 容器」，retrieval 邏輯不依賴它**。

#### 4.3.1 Retrieval 邏輯（明確定義）

當用戶在 dashboard 切到客戶 X 做檢索（不論 X 是 `_personal` / `aicycle` / 外部客戶 `client-foo`）：

```
retrieved_cases = cases WHERE client = X
                  UNION
                  cases WHERE client.type = 'self'   # 所有自己品牌 union 進來，當 reference 池
```

舉例：
- 切到 `aicycle`（type: self）→ retrieve = aicycle + _personal + zhenheco（其他 type:self） + 任何 type:self 的 client
- 切到 `client-foo`（type: client）→ retrieve = client-foo + _personal + aicycle + zhenheco（type:self 全 union）
- 切到 `_personal`（type: self）→ retrieve = _personal + aicycle + zhenheco（_personal 自己 + 其他 type:self；不 union 任何 type:client）

**不會發生**：客戶 A（type: client）的 case 不會被用在客戶 B（type: client）的設計檢索上。客戶間隔離靠 type:client 互斥。
...
-- 跨客戶共用 markdown 文件（personal-style-guide.md + scenario-overrides/*.md）
-- 用於 chokidar 偵測這些檔變動時做 hash 比對 + cache invalidation
CREATE TABLE IF NOT EXISTS documents (
  kind TEXT NOT NULL,                        -- 'style-guide' | 'scenario-override'
  scenario TEXT,                              -- only set when kind='scenario-override' (e.g. 'landing')
...
**前置**：`gemini-rotate.sh` 目前不支援 image input。**Phase D 第一個 commit** 必須先擴此腳本加 `--image <path>` flag（或 base64 stdin pipe 方案，phase D spike 階段二選一），對 6 帳號 rotate / model fallback 邏輯保持向後相容。下方程式碼假設此前置已完成。

```typescript
// lib/vision/gemini-rotate.ts（phase D 完成 gemini-rotate.sh 擴充後的最終目標 API）
...
export const geminiAdapter: VisionAdapter = {
  ...
  async extractTokens(imagePath) {
    // 注意：--image flag 是 phase D 擴充的前置條件
...
4. **Phase D**：Vision adapter（gemini-rotate + Anthropic） + 測試
   - **Phase D 第一個 commit 必須**：擴 `~/.claude/skills/gemini-agent/scripts/gemini-rotate.sh` 加 `--image <path>` flag（或 base64 stdin 方案二選一），且必須對既有 6 帳號 rotate / model fallback / `should_rotate` 訊號保持向後兼容。寫測試確認原有 text-only 行為不變。
   - 之後再寫 `lib/vision/gemini-rotate.ts` adapter
